function editpupil(id, lastname, firstname, pclass, card_id, group) {
	var req = new ajax();
	var url = "handle.php?do=editpupil&id=" + id + "&lastname=" + lastname + "&firstname=" + firstname + "&class=" + pclass + "&card_id=" + card_id + "&group=" + group;
	//url = encodeURI(url);
	//alert(url);
	req.create(url);
	resp = req.fetch();
...
function process() {
	...
	if(!valid) {
		...
		if(editpupil(document.forms['add'].elements['id'].value, encodeURIComponent(document.forms['add'].elements['lastname'].value), encodeURIComponent(document.forms['add'].elements['firstname'].value), encodeURIComponent(document.forms['add'].elements['class'].value), document.forms['add'].elements['card_id'].value, document.forms['add'].elements['group'].value)) {
			window.top.list();
			alert("Schueler geandert");
